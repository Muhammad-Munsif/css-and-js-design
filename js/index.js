...
function renderElement(elementData) {
    ...
    deleteBtn.className = 'delete-btn absolute top-2 right-2 text-red-500 hover:text-red-600 dark:text-red-400 dark:hover:text-red-300 transition opacity-0 group-hover:opacity-100 bg-white dark:bg-gray-800 rounded-full w-8 h-8 flex items-center justify-center z-10 shadow';
    ...
    infoBadge.className = 'info-badge absolute top-2 left-2 bg-white/80 dark:bg-gray-800/80 text-xs text-gray-600 dark:text-gray-300 px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition';
...
function clearAll() {
    ...
    if (confirm('Are you sure you want to delete ALL elements? This action cannot be undone.')) {
        elements = [];
...
function updateCodePreview() {
    ...
    codePreview.innerHTML = codePreview.textContent
        ...
        .replace(/background-color:/g, '<span class="text-blue-600">background-color:</span>')
...
function showToast(message, type = 'success') {
    ...
    const colorClasses = ['bg-green-600', 'bg-red-600', 'bg-yellow-600', 'bg-blue-600', 'bg-purple-600'];
